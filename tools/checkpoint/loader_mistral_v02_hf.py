...
def add_arguments(parser):
    group = parser.add_argument_group(title='Mistral-v0.2 HF loader.')
...
def load_args_from_checkpoint(args):
    ...
    # Read mistral args.
    try:
        # loading configs from HuggingFace
        from transformers import MistralConfig
        mistral_args = MistralConfig.from_pretrained(args.load).to_dict()
    except:
    ...
    # args.tokenizer_type = "mistral2Tokenizer"
    args.bf16 = True
    ...
    args.mistral = mistral_args
    args.ffn_hidden_size = mistral_args["intermediate_size"]

    if "num_key_value_heads" in mistral_args:
...
def load_checkpoint_to_model(args):
    ...
    hf_model = MistralForCausalLM.from_pretrained(args.load)#, device_map="cpu")
...
def _load_checkpoint(queue, args):
    ...
    # mistral-2 requires HF transformers >=4.31.0.
    ...
    assert args.model_type == 'GPT', 'Mistral is a GPT model.'
